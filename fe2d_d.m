function fe2d_d
% 'fe2d_d.m'   2D finite element Matlab code for Scheme 2 applied
...
% Identify the boundary nodes on Gamma
bn = unique(edges(:));
% Number of nodes (isn) on Gamma
[junk,isn]=size(bn);
...
for nt=1:N
    ...
    rhs_u = u + delt*F;
    rhs_v = v + delt*G;
    % Impose dirichlet boundary conditions on Gamma
    for i = 1:isn
        node = bn(i);
        xx = p(1,node);
        yy = p(2,node);
        B1(node,:)=0; 
        B1(node,node)=1;
        rhs_u(node)=gu(xx,yy,tn);
        B2(node,:)=0; 
        B2(node,node)=1;
        rhs_v(node)=gv(xx,yy,tn);
    end
